...
type HeartbeatManager interface {
	...
	// GetNodeStats 获取节点统计信息
	GetNodeStats(nodeID string) (*NodeStats, bool)
	// ListOnlineNodes 列出所有在线节点
	ListOnlineNodes() []string
	// RemoveNode 移除节点
...
type MetaDataManager interface {
	...
	SaveClusterInfo(ctx context.Context, info *ClusterInfo) error
	// LoadClusterInfo 加载集群信息
	LoadClusterInfo(ctx context.Context) (*ClusterInfo, error)
	// SaveDNodeInfo 保存物理节点信息
	SaveDNodeInfo(ctx context.Context, info *DNodeInfo) error
	// LoadDNodeInfo 加载物理节点信息
	LoadDNodeInfo(ctx context.Context, nodeID string) (*DNodeInfo, error)
	// ListDNodeInfo 列出所有物理节点信息
	ListDNodeInfo(ctx context.Context) ([]*DNodeInfo, error)
	// SaveMNodeInfo 保存虚拟管理节点信息
	SaveMNodeInfo(ctx context.Context, info *MNodeInfo) error
	// LoadMNodeInfo 加载虚拟管理节点信息
	LoadMNodeInfo(ctx context.Context, nodeID string) (*MNodeInfo, error)
	// ListMNodeInfo 列出所有虚拟管理节点信息
	ListMNodeInfo(ctx context.Context) ([]*MNodeInfo, error)
	// SaveVNodeInfo 保存虚拟数据节点信息
	SaveVNodeInfo(ctx context.Context, info *VNodeInfo) error
	// LoadVNodeInfo 加载虚拟数据节点信息
	LoadVNodeInfo(ctx context.Context, nodeID string) (*VNodeInfo, error)
	// ListVNodeInfo 列出所有虚拟数据节点信息
	ListVNodeInfo(ctx context.Context) ([]*VNodeInfo, error)
	// SaveVGroupInfo 保存虚拟数据节点组信息
	SaveVGroupInfo(ctx context.Context, info *VGroupInfo) error
	// LoadVGroupInfo 加载虚拟数据节点组信息
	LoadVGroupInfo(ctx context.Context, groupID string) (*VGroupInfo, error)
	// ListVGroupInfo 列出所有虚拟数据节点组信息
	ListVGroupInfo(ctx context.Context) ([]*VGroupInfo, error)
	// SaveDatabaseClusterInfo 保存数据库集群信息
	SaveDatabaseClusterInfo(ctx context.Context, info *DatabaseClusterInfo) error
	// LoadDatabaseClusterInfo 加载数据库集群信息
	LoadDatabaseClusterInfo(ctx context.Context, dbName string) (*DatabaseClusterInfo, error)
	// ListDatabaseClusterInfo 列出所有数据库集群信息
	ListDatabaseClusterInfo(ctx context.Context) ([]*DatabaseClusterInfo, error)
	// SaveTableClusterInfo 保存表集群信息
	SaveTableClusterInfo(ctx context.Context, info *TableClusterInfo) error
	// LoadTableClusterInfo 加载表集群信息
	LoadTableClusterInfo(ctx context.Context, dbName, tableName string) (*TableClusterInfo, error)
	// ListTableClusterInfo 列出数据库中所有表集群信息
	ListTableClusterInfo(ctx context.Context, dbName string) ([]*TableClusterInfo, error)
}
