...
func BenchmarkXOREncoder(b *testing.B) {
	// 准备测试数据
...
func BenchmarkXORDecoder(b *testing.B) {
	// 准备测试数据
	data := make([]float64, 1000)
	for i := range data {
		data[i] = float64(i) * 0.1
...
func BenchmarkCompressionRatioXOR(b *testing.B) {
	testCases := []struct {
		name string
		data []float64
	}{
	...
	for _, tc := range testCases {
		b.Run(tc.name+"_XOREncoder", func(b *testing.B) {
			// 原始大小
		...
		b.Run(tc.name+"_SimpleValueEncoder", func(b *testing.B) {
			// 原始大小
			originalSize := len(tc.data) * 8 // 每个float64占8字节
